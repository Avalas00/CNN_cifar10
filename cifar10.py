...
train_data = torchvision.datasets.CIFAR10("data", train=True, transform=transformation, download=True)

test_data = torchvision.datasets.CIFAR10("data", train=False, transform=transformation, download=True)
...
model.fc = nn.Linear(num_ftrs, 10)  # 將輸出層改為 CIFAR-10 的類別數量
...
# model = CIFAR10()
model = torch.load(f"cifar10_{best_model}.pth", map_location=torch.device("cpu"))
# model = torch.load(f"cifar10_22.pth", map_location=torch.device("cpu"))
def imageshow(img):
  img = img/2 + 0.5
  npimg = img.numpy()
...
image_path ="./deeplearning/ai_dog.jpeg"
image =Image.open(image_path)
print(image)
...
model = torch.load("cifar100_100.pth", map_location=torch.device("cpu"))
# print(model)
image = torch.reshape(image, (1, 3, 32, 32))
model.eval()#transfer to test mode
with torch.no_grad():
